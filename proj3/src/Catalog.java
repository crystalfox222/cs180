...
public class Catalog {
    ...
    public void writeCatalog(File bookCatalog) throws FileNotFoundException {
        ...
        for (a = 0; a < catalog.size(); a++) {
            ...
            String isCheckedOut = "";
            if (catalog.get(a).isCurrentlyLoaned() == true) {
                isCheckedOut = "Yes";
            } else {
                isCheckedOut = "No";
            }
            ...
            pw.println(catalog.get(a).getId() + ", " + catalog.get(a).getTitle() +
                    ", " + catalog.get(a).getAuthorFirstName()
                    ...
                    + ", " + catalog.get(a).getNumCheckouts() + ", " +
                    String.format("%.2f", catalog.get(a).getPurchasePrice()) + ", "
                    + isCheckedOut);
        }
        pw.close();
    ...
    public boolean removeBook(int id) {
        ...
        if (check == 1) {
            this.catalog.remove(catalog.get(i));
            return true;
        } else {
            return false;
        }
    ...
    public ArrayList<Book> findBooks(String search) {
        ...
        for (int i = 0; i < catalog.size(); i++) {
            if (catalog.get(i).getTitle().toLowerCase().contains(search.toLowerCase())) {
                list.add(catalog.get(i));
            } else if (catalog.get(i).getAuthorFirstName().toLowerCase().contains(search.toLowerCase())) {
                list.add(catalog.get(i));
            } else if (catalog.get(i).getAuthorLastName().toLowerCase().contains(search.toLowerCase())) {
                list.add(catalog.get(i));
            } else if (catalog.get(i).getPublisher().toLowerCase().contains(search.toLowerCase())) {
                list.add(catalog.get(i));
            } else if (catalog.get(i).getPublisherCity().toLowerCase().contains(search.toLowerCase())) {
                list.add(catalog.get(i));
            }
        }
        return list;
    ...
    public void printCatalogToFile(int mode, File newFile) throws FileNotFoundException {
        ...
        if (mode == 1) {
            ...
            for (i = 0; i < catalog.size(); i++) {
                if (catalog.get(i).isCurrentlyLoaned() == true) {
                    isCheckedOut = "Yes";
                } else {
                    isCheckedOut = "No";
                }
